...
public:
  ...
  Lbfgsb() : c__1(1), c__11(11), _n(0), _m(5), _f(HUGE_VAL), 
	     _factr(1.0e7), _pgtol(1.0e-3), _maxit(100), 
	     _iprint(0), _fncount(0), _grcount(0), _fdfcount(0), _fail(0), _best_fn(HUGE_VAL),_count_0(0) {}
  ...
  void set_initial_point(const V& x) {_x = x; _n = x.size();}
  ...
  L111:
    ...
    if (iprint >= 1) print_message("At iterate %5d  f= %12.5g  |proj g|= %12.5g\n",
				   iter, *f, sbgnrm);
  ...
  void prn2lb(int n, double *x, double *f, double *g, int iprint,
	...
	pvector("G =", g, n);
      ...
      print_message("At iterate %5d  f = %12.5g  |proj g|=  %12.5g\n",
	      iter, *f, sbgnrm);
    }
  }
  void prn3lb(int n, double *x, double *f, char *task, int iprint,
    ...
    if (iprint >= 0) {
      switch(info) {
      case -1: print_message("Matrix in 1st Cholesky factorization in formk is not Pos. Def."); break;
      case -2: print_message("Matrix in 2st Cholesky factorization in formk is not Pos. Def."); break;
      case -3: print_message("Matrix in the Cholesky factorization in formt is not Pos. Def."); break;
      case -4: print_message("Derivative >= 0, backtracking line search impossible."); break;
      case -5: print_message("l(%d) > u(%d).  No feasible solution", k, k); break;
      case -6: print_message("Input nbd(%d) is invalid", k); break;
      case -7: print_message("Warning:  more than 10 function and gradient evaluations\n"
			     "  in the last line search\n"); break;
      case -8: print_message("The triangular system is singular."); break;
      case -9: print_message("%s\n%s\n", 
			     "Line search cannot locate an adequate point after 20 function", 
			     "and gradient evaluations"); break;
      default: break;
      }
    }
    */
